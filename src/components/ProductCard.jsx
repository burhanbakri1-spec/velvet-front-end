...
import { getCategoryLabel, getProductBadge, getProductDescription, getProductName } from '../data/products';
...
export default function ProductCard({ product, onAddToCart }) {
  ...
  return (
    <article className="product-card">
      <Link className="product-card__media" to={`/products/${product.slug}`} aria-label={`${copy.products.view} ${name}`}>
        {badge && <span className="product-card__badge">{badge}</span>}
        <img className="product-card__image product-card__image--primary" src={product.image} alt={name} />
      ...
      <div className="product-card__body">
        ...
        <Link to={`/products/${product.slug}`}><h2>{name}</h2></Link>
        ...
        <div className="product-card__footer">
          ...
          {onAddToCart ? (
            ...
            <Link className="product-card__arrow" to={`/products/${product.slug}`} aria-label={`${copy.products.open} ${name}`}>{locale === 'ar' ? '←' : '→'}</Link>
          )}
        </div>
      </div>
    </article>
