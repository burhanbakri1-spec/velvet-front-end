...
test('BrandPage hero uses full-width adaptive media without hero logo', () => {
  ...
  assert.doesNotMatch(brandPage, /category-hero__bg/);
  assert.match(styles, /\.category-hero\s*\{[^}]*aspect-ratio:\s*32\s*\/\s*15/);
  assert.match(styles, /\.category-hero\s*\{[^}]*overflow:\s*hidden/);
  assert.doesNotMatch(styles, /\.category-hero__bg/);
  ...
  assert.doesNotMatch(styles, /\.category-hero\s*\{[^}]*height:\s*min\(/);
  assert.match(styles, /\.brand-hero \.category-hero__media[\s\S]*height:\s*100%/);
  assert.match(styles, /\.brand-hero\s*\{[^}]*aspect-ratio:\s*32\s*\/\s*15/);
  assert.doesNotMatch(styles, /\.brand-hero \.category-hero__bg/);
  assert.match(styles, /\.brand-showcase\.brand-showcase--full-banner[\s\S]*aspect-ratio:\s*32\s*\/\s*15/);
  assert.doesNotMatch(styles, /\.brand-showcase\.brand-showcase--full-banner \.brand-showcase__bg/);
...
test('homepage BrandShowcase uses full-banner natural media without cover crop', () => {
  ...
  assert.doesNotMatch(showcase, /brand-showcase__bg/);
  assert.match(styles, /\.brand-showcase\.brand-showcase--full-banner[\s\S]{0,220}?aspect-ratio:\s*32\s*\/\s*15/);
